...
def px4_selection_intent_is_published(state, selection, now):
    """Admit an old preview as intent only; worker must revalidate fresh data."""
    history = state.get('selection_history', [])
    if not isinstance(history, list) or len(history) > 8:
        return False
    for item in history:
        if not isinstance(item, dict) or item.get('sequence') != selection['sequence']:
            continue
        captured = item.get('capture_time_s')
        return (isinstance(captured, (int, float)) and not isinstance(captured, bool)
                and math.isfinite(captured) and 0 <= now-captured <= 3.
                and selection['track_id'] in item.get('selectable_tracks', []))
    return False
...
class StudioManager:
    ...
    def start(self, value):
        ...
        with self.lock:
            ...
            write_json(folder/'state.json', dict(phase='loading', config=config, sim_s=0.,
                backend=config['backend'], simulation_only=True, message='Starting '+
                ('owned PX4/SIH simulation' if config['backend'] == 'px4_sih' else 'MuJoCo simulation')))
            environment = dict(os.environ, PYTHONDONTWRITEBYTECODE='1', OMP_NUM_THREADS='2')
            module = 'experiments.px4_studio' if config['backend'] == 'px4_sih' else 'experiments.mantis_session'
            self.process = subprocess.Popen([sys.executable, '-B', '-u', '-m', module,
    ...
    def command(self, value):
        ...
        with self.lock:
            ...
            if 'operation' in value:
                ...
                if current['backend'] == 'px4_sih' and value['operation'] == 'pause':
                    raise ValueError('PX4 cannot pause its dynamics; use Land & save')
            ...
            if 'settings' in value:
                ...
                new['settings'] = ({} if current['backend'] == 'px4_sih'
                                   else {k: updated[k] for k in LIVE_KEYS})
            if 'selection' in value:
                ...
                if selection.get('clear') is True:
                    ...
                    if current['backend'] == 'px4_sih':
                        if not px4_selection_intent_is_published(state, selection, time.monotonic()):
                            raise ValueError('Selected preview expired or person was not selectable; choose from a newer frame')
                    elif selection['sequence'] != state.get('selection', {}).get('sequence'):
    ...
    def _state(self, identity):
        ...
        if state.get('backend') == 'px4_sih':
            capture = (state.get('frame') or {}).get('capture_time_s')
            state['frame_age_s'] = (max(0., time.monotonic()-capture)
                if isinstance(capture, (int, float)) and not isinstance(capture, bool)
                and math.isfinite(capture) else None)
    ...
    def close(self):
        with self.lock:
            if self.running():
                ...
                if self.backend == 'px4_sih':
                    # PX4 may need 30 seconds for confirmed LAND, then bounded
                    # encoder cleanup. Never kill its wrapper before it can
                    # stop the separately owned simulator child.
                    try:
                        self.process.wait(timeout=120)
                    except subprocess.TimeoutExpired as exc:
                        raise RuntimeError('PX4 landing/cleanup has not finished; its owned runner remains active') from exc
                    return
                try:
