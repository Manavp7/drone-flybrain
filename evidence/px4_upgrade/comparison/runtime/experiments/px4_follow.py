...
def run(folder, smoke=False, selected_track=1, low_noise_sensors=False, *,
    ...
    def teardown_event(name, **values):
        try:
            event(name, **values)
        except BaseException as exc:
            # event() appends the original receipt before attempting output.
            # A second interrupt in reporting cannot skip independent cleanup.
            events.append(dict(event='teardown_reporting_failed',
                elapsed_s=time.monotonic()-epoch, error=f'{type(exc).__name__}: {exc}'))
    try:
        if not smoke:
        ...
        if not smoke:
            ...
            recorder = AsyncRecorder(SessionRecorder(folder/'recording' if studio is None else
                studio.recording_folder, fps=5, max_bytes=(32 if studio is None else
        ...
        while (time.monotonic()-follow_start < (hover_seconds if smoke else duration_s if mission else 24)
            ...
            if camera:
                try:
                    if request['forward_speed'] > 0:
                        # The guardian's declared sensor period is 50 ms. Keep
                        # original capture metadata; every send still enforces
                        # 100 ms maximum depth/state age. Never renew a cache.
                        if cached_safety is None or now-last_safety_render >= .05:
                            cached_safety = camera.capture(link, state, safety=True)
                            last_safety_render = now
                        safety_capture = cached_safety.capture_time_s
                        depth = guardian.check(cached_safety, state, request['forward_speed'], time.monotonic())
                    else:
                        # A held request cannot gain movement authority from a
                        # missing depth frame. Avoid rendering just to send zero.
                        depth = dict(forward_speed=0., reason='no_forward_request')
            ...
            if recorder and last_rgb is not None and now-last_record >= .2:
            ...
            if not smoke and final_request['forward_speed'] > 0 and (safety_capture is None or sent_at-safety_capture > .1
        ...
        if link is not None and armed_requested:
            try:
                ...
                teardown_event('landed_disarmed')
            except BaseException as exc:
                failure = failure or 'Landing confirmation unavailable'
                teardown_event('landing_unconfirmed', error=f'{type(exc).__name__}: {exc}')
        try:
        ...
        for name, resource in [('link', link), ('px4', lease), ('vision', worker),
            ...
            if resource is not None:
                try:
                    ...
                    result.update(passed=False, error=result['error'] or f'{name} cleanup failed: {error_text}')
                    teardown_event('cleanup_failed', resource=name, error=error_text)
